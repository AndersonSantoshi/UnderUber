  ...
  dayRides.forEach(function(ride) {
    ...
    div.innerHTML =
      '<div style="display:flex; justify-content:space-between; align-items:center; width:100%;">' +
        ...
        '<span style="font-size:12px; color:#888; text-align:center;">' + formatDate(ride.date) + '</span>' +
        '<span class="ride-value">R$ ' + ride.valorBruto.toFixed(2).replace('.', ',') + '</span>' +
      '</div>' +
      '<div class="ride-profit ' + (result.lucroViagem >= 0 ? 'positive' : 'negative') + '" style="margin-left:10px;">' +
  ...
  dayRides.forEach(function(ride) {
    ...
    div.innerHTML =
      '<div style="flex:1;">' +
        ...
        '<div style="font-size:12px;color:#999;">' + formatDate(ride.date) + '</div>' +
      '</div>' +
...
function generatePDF() {
  ...
  html += '<style>';
  html += 'body { font-family: "Helvetica", Arial, sans-serif; padding: 30px; background: #0A0A0A; }';
  html += '.container { max-width: 800px; margin: 0 auto; background: #1A1A1A; padding: 30px; border-radius: 20px; border: 1px solid #2A2A2A; }';
  html += '.logo { text-align: center; margin-bottom: 20px; }';
  html += '.logo img { max-width: 200px; height: auto; }';
  html += 'h1 { color: #FF8C00; text-align: center; font-size: 24px; margin-bottom: 5px; }';
  html += '.subtitle { text-align: center; color: #888; font-size: 14px; margin-bottom: 30px; }';
  html += '.header { text-align: center; margin-bottom: 30px; border-bottom: 2px solid #FF8C00; padding-bottom: 15px; }';
  html += '.header small { color: #666; }';
  html += 'table { width: 100%; border-collapse: collapse; margin: 20px 0; font-size: 13px; }';
  html += 'th { background: #FF8C00; color: #0A0A0A; padding: 10px 8px; text-align: left; }';
  html += 'td { padding: 8px; border-bottom: 1px solid #2A2A2A; color: #FFFFFF; }';
  html += 'tr:nth-child(even) { background: #0A0A0A; }';
  html += '.resumo { background: #0A0A0A; padding: 15px 20px; border-radius: 10px; margin: 20px 0; border-left: 4px solid #FF8C00; }';
  html += '.resumo h3 { margin: 0 0 10px 0; color: #FF8C00; }';
  html += '.resumo p { margin: 5px 0; font-size: 14px; color: #FFFFFF; }';
  html += '.resumo .value { font-weight: bold; color: #FF8C00; }';
  html += '.footer { text-align: center; color: #666; font-size: 12px; margin-top: 30px; border-top: 1px solid #2A2A2A; padding-top: 15px; }';
  html += '.positive { color: #4CAF50; }';
  html += '.negative { color: #FF4444; }';
  html += '.btn-voltar { display: block; width: 100%; padding: 14px; background: #FF8C00; color: #0A0A0A; border: none; border-radius: 12px; font-size: 16px; font-weight: 700; cursor: pointer; margin-top: 20px; text-align: center; }';
  html += '.btn-voltar:hover { background: #E67E22; }';
  html += '.no-print { display: block; }';
  html += '@media print { .no-print { display: none !important; } .container { border: none !important; } body { background: white; padding: 10px; } }';
  html += '</style></head><body>';
  
  html += '<div class="container">';
  
  html += '<div class="logo">';
  html += '<img src="https://AndersonSantoshi.github.io/UnderUber/icon-512.png" alt="UnderUber" style="max-width:180px; height:auto;">';
  html += '</div>';
  
  html += '<div class="header">';
  html += '<h1>Relatório de Corridas</h1>';
  ...
  html += '<h3 style="color:#FF8C00; margin-top:25px;">📋 DETALHES DAS CORRIDAS</h3>';
  html += '<table>';
